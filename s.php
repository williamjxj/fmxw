...
if (isset($_GET['q'])) {
	...
	if(empty($_GET['q'])) {
		...
		//$key = '@title "'.$q.'" @* (丑闻|曝光|腐|贿|淫|荡|娼|滥|罪|恶|贬|讹|劫|燥|偷|窃|狡|诈|毒|害|坏|奸污|诱惑|贪污|魔鬼|过期|变质|掺假|邪恶|疾病|落后|贪心|自私|蛮荒|贪婪|姑息|愚昧|假冒|欺诈|悲剧|消极|放纵|虚假|欺骗|倒台|麻烦|厌烦|危机|霉运|倒霉|温床|腐败|谎言|试探|次品|舆论|绯闻|露点|情妇|小三|流氓|恶霸|土匪|在逃|犯罪|辩解|暴发户|卖国|浪尖|愤怒|逃避|媒体|作恶|作秀|负面|新闻|评价|真相)';
		//" @content "'.$q.'"/2
		//中枪，双规，撤职，质疑，威胁，风险,污染
		$key ='@title "'.$q.'" @* (丑闻|曝光|乱|腐|贿|淫|荡|娼|滥|罪|恶|霸|贬|讹|劫|燥|偷|癌|窃|狡|诈|毒|害|坏|谎|病|贪|奸|苦|哭|污|诱|惑|魔鬼|过期|变质|假|邪|落后|自私|蛮荒|姑息|愚昧|欺|诈|悲剧|消极|放纵|虚假|倒台|烦|危机|霉|温床|败|试探|次品|舆论|绯闻|露点|情妇|小三|流氓|辩解|卖国|浪尖|愤怒|逃避|媒体|作秀|负面|新闻|评价|真相|障碍|恐慌|暧昧|流言|话题|炒作|不公|扒光|暴发)';
		...
		$weights = array('title'=>80, 'content'=>2);
    ...
    if(empty($q)) {
        ...
        switch($_SESSION[PACKAGE][SEARCH]['sort']) {
            ...
            case 1:
                ...
                break;
            case 'cate_id':
				$obj->cl -> SetFilter('cate_id', array($_SESSION[PACKAGE][SEARCH]['sort']));
                $obj->cl->SetSortMode(SPH_SORT_EXTENDED, "@relevance DESC, @id DESC");
				break;
            case 'iid':
				$obj->cl -> SetFilter('iid', array($_SESSION[PACKAGE][SEARCH]['sort']));
                $obj->cl->SetSortMode(SPH_SORT_EXTENDED, "@relevance DESC, @id DESC");
                break;
            case 'created':
                $obj->cl->SetSortMode(SPH_SORT_TIME_SEGMENTS, 'created');
 ...
 */
if(empty($weights)) {
	$weights = array('title'=>80, 'content'=>1);
	$obj->cl->SetFieldWeights( $weights );
}
